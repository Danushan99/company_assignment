...
import React, { memo, useContext, useMemo, useCallback, useRef, useEffect } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { matchRoutes, useLocation } from 'react-router-dom';
import GlobalStyles from '@mui/material/GlobalStyles';
import { alpha } from '@mui/material/styles';
import { getNewMenuArray } from 'app/store/fuse/navigationSlice';
import { getwordsList } from 'app/main/backend-app/administrator/store/wordsTranslaterSlice';
...
function FuseLayout(props) {
	...
	// tranlate common path
	const curretPath = '/menu/common';
	const encodedCurretPath = btoa(curretPath);

	useEffect(() => {
		if (window.location.pathname !== '/login') {
			dispatch(getNewMenuArray());
			dispatch(getwordsList(encodedCurretPath));
		}
	}, [dispatch]);
