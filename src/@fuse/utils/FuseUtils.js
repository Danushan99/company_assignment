...
class FuseUtils {
	...
	static formatNumberWithTwoDecimalsAndCommas(number) {
		// Check if the input is a number or can be converted to a number
		if (typeof number !== 'number') {
		  number = parseFloat(number);
		  if (isNaN(number)) {
			// If it's not a valid number, return an empty string or handle the error as needed
			return '';
		  }
		}
	
		// Use toFixed(2) to round to two decimal places and then use regex to add commas
