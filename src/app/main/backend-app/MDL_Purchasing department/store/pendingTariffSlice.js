...
export const getAllTarrif = createAsyncThunk(
	...
	async ({ id, params }, { rejectWithValue }) => {
		try {
			const response = await axios.get(`${apiUrl}/${MDLdepartmentUrl.pendingTariffUrl}/${id}`, { params });
...
export const sendEmail = createAsyncThunk('MDLApp/gettariffs/sendEmail', async ({ params }, { rejectWithValue }) => {
	try {
		//  const response = await axios.get(`${apiUrl}/${MDLdepartmentUrl.getarrifUrl}/${id}`, { params });
		const response = await axios.post(`${apiUrl}/${MDLdepartmentUrl.sendEmail}`, { params });
		console.log(params);
		if (response.status === 200) {
			const data = response.data?.datas || [];
			console.log(data);
