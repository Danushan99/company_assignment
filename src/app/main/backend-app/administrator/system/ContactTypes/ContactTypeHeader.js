...
import { setContainerTypeSearchText, OpenNewContainerTypeDialog } from './store/ContactTypesSlice';

function ContactTypeHeader(props) {
	...
	return (
		<div className="flex flex-1 w-full items-center justify-between">
				...
				<ThemeProvider theme={mainTheme}>
					...
					>
						...
						<Input
							...
							onChange={ev => dispatch(setContainerTypeSearchText(ev))}
						/>
					</Paper>
				</ThemeProvider>
			</div>
			<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
				<Button
					onClick={() => {
						dispatch(OpenNewContainerTypeDialog());
					}}
