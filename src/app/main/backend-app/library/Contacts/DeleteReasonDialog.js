...
import { removeContacts, closeViewContactDetailDialog } from './store/contactsSlice';
import {
	removeContactEmploye,
	removeContactPerson,
	removeContactAgreement,
	removeContactCompany,
	removeContactAddress
} from './store/singleContactSlice';
...
function DeleteReasonDialog(props) {
	const { openDeleteReasonDialog, selectedIds, selectedKey, setOpenDeleteReasonDialog, deleteSection } = props;
	...
	function onSubmit(data) {
		if (data?.reason === '...' || data?.reason === '') {
			...
			if (openDeleteReasonDialog && deleteSection === '') {
				const submitData = {
					reason: data?.reason,
					ids: selectedIds
				};
				dispatch(removeContacts(submitData));
			} else if (openDeleteReasonDialog && deleteSection === 'personalDetails') {
				dispatch(removeContactEmploye(submitKeyData));
			} else if (openDeleteReasonDialog && deleteSection === 'contactAgreement') {
				dispatch(removeContactAgreement(submitKeyData));
			} else if (openDeleteReasonDialog && deleteSection === 'contactAddress') {
				dispatch(removeContactAddress(submitKeyData));
			} else if (openDeleteReasonDialog && deleteSection === 'contactPerson') {
				dispatch(removeContactPerson(submitKeyData));
			} else if (openDeleteReasonDialog && deleteSection === 'basicInfoOtherCompany') {
				dispatch(removeContactCompany(submitKeyData));
			}
		}
		handleClose();
	}
