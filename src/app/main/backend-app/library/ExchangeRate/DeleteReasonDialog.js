...
import { removeExchangeRate, getExchangeRateList } from './store/exchangeRateSlice';
...
function DeleteReasonDialog(props) {
	const { data, setData, openDeleteReasonDialog, selectedIds, setOpenDeleteReasonDialog } = props;
	...
	function onSubmit(formdata) {
		if (formdata?.reason === '...' || formdata?.reason === '') {
			...
			if (openDeleteReasonDialog === true) {
				dispatch(removeExchangeRate(submitData)).then(response => {
					dispatch(getExchangeRateList()).then(responseall => {
